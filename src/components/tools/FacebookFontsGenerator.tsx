...
import { facebookFontStyles } from '@/lib/textTransforms';
import { Check } from 'lucide-react';

export function FacebookFontsGenerator() {
  ...
  useEffect(() => {
    if (text) {
      const style = facebookFontStyles.find(s => s.id === selectedStyle);
      if (style) {
  ...
  return (
    <BaseTextConverter
      title={tool('facebookFonts.title')}
      description={tool('facebookFonts.description')}
      inputLabel={common('labels.inputText')}
      outputLabel={tool('facebookFonts.outputLabel')}
      ...
      uploadDescription=""
      downloadFileName={tool('facebookFonts.downloadFileName')}
    ...
    >
      {/* Facebook Font Style Selector */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-foreground mb-4">
          {tool('facebookFonts.selectStyle', 'Select Facebook Font Style')}
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {facebookFontStyles.map((style) => (
            <button
              key={style.id}
