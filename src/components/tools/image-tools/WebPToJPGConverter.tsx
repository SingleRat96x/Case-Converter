...
const getQualityPresets = (tool: (key: string) => string) => [
  { label: tool('webpToJpg.options.qualityLevels.max'), value: 1.0 },
  { label: tool('webpToJpg.options.qualityLevels.high'), value: 0.9 },
  { label: tool('webpToJpg.options.qualityLevels.medium'), value: 0.7 },
  { label: tool('webpToJpg.options.qualityLevels.low'), value: 0.5 }
];
...
export function WebPToJPGConverter() {
  ...
  const convertSingleFile = useCallback(async (file: File): Promise<Blob> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!file.type.match(/^image\/webp$/i)) {
        reject(new Error(tool('webpToJpg.errors.invalidFile')));
        return;
      }

      if (file.size > 10 * 1024 * 1024) {
        reject(new Error(tool('webpToJpg.errors.fileTooLarge')));
        return;
      }
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          try {
            ...
            if (!canvas) {
              reject(new Error(tool('webpToJpg.errors.canvasNotSupported')));
              return;
            }
            ...
            canvas.toBlob(
              (blob) => {
                if (blob) {
                  ...
                  reject(new Error(tool('webpToJpg.errors.failedToConvert')));
                }
              },
              'image/jpeg',
              options.quality
        ...
        img.onerror = () => {
          reject(new Error(tool('webpToJpg.errors.failedToLoad')));
        };
      ...
      reader.onerror = () => {
        reject(new Error(tool('webpToJpg.errors.failedToRead')));
      };
  ...
  const processFileQueue = useCallback(async (queue: ProcessedFile[]) => {
    ...
    for (const file of pendingFiles) {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : tool('webpToJpg.errors.conversionFailed');
  ...
  const handleFileUpload = useCallback(async (files: File[]) => {
    ...
    if (!shouldUseBulkMode && files.length === 1) {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : tool('webpToJpg.errors.failedToConvert');
      ...
      if (validFiles.length === 0) {
        setError(tool('webpToJpg.errors.invalidFile'));
        return;
      }
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(tool('webpToJpg.errors.failedToCreateZip'));
    }
  ...
  return (
    <div className="space-y-6">
      <BaseBulkConverter
        title={tool('webpToJpg.title')}
        description={tool('webpToJpg.description')}
        uploadAreaLabel={tool('webpToJpg.labels.uploadArea')}
        dragDropLabel={tool('webpToJpg.labels.dragDrop')}
        supportedFormatsLabel={tool('webpToJpg.labels.supportedFormats')}
        clearText={common('buttons.clear')}
        downloadAllText="Download All"
        downloadFileName={tool('webpToJpg.downloadFileName')}
      ...
      >
        {/* WebP to JPG Options Accordion */}
        <Accordion className="w-full">
          <AccordionItem 
            title={tool('webpToJpg.options.conversionSettings')}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('webpToJpg.options.quality')}</h3>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('webpToJpg.options.compression')}</h3>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['fast', 'standard', 'best'] as const).map((compression) => (
                    <Button
                      ...
                      className="w-full"
                    >
                      {tool(`webpToJpg.options.compressionLevels.${compression}`)}
                    </Button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('webpToJpg.options.backgroundColor')}</h3>
                </div>
