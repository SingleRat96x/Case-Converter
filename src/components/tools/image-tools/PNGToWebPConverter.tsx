...
const getQualityPresets = (tool: (key: string) => string) => [
  { label: tool('pngToWebp.options.qualityLevels.max'), value: 1.0 },
  { label: tool('pngToWebp.options.qualityLevels.high'), value: 0.9 },
  { label: tool('pngToWebp.options.qualityLevels.medium'), value: 0.7 },
  { label: tool('pngToWebp.options.qualityLevels.low'), value: 0.5 }
];
...
export function PNGToWebPConverter() {
      ...
      try {
        ...
        const convertedBlob = await (async (fileToProcess: File): Promise<Blob> => {
          return new Promise((resolve, reject) => {
            if (!fileToProcess.type.match(/^image\/png$/i)) {
              reject(new Error(tool('pngToWebp.errors.invalidFile')));
              return;
            }

            if (fileToProcess.size > 10 * 1024 * 1024) {
              reject(new Error(tool('pngToWebp.errors.fileTooLarge')));
              return;
            }
            ...
            reader.onload = (e) => {
              ...
              img.onload = () => {
                try {
                  ...
                  if (!canvas) {
                    reject(new Error(tool('pngToWebp.errors.canvasNotSupported')));
                    return;
                  }
                  ...
                  canvas.toBlob(
                    (blob) => {
                      if (blob) {
                        ...
                        reject(new Error(tool('pngToWebp.errors.failedToConvert')));
                      }
                    },
                    'image/webp',
                    options.lossless ? 1.0 : options.quality
                  );
                } catch {
                  reject(new Error(tool('pngToWebp.errors.failedToConvert')));
                }
              ...
              img.onerror = () => reject(new Error(tool('pngToWebp.errors.failedToLoad')));
              img.src = imageUrl;
            };

            reader.onerror = () => reject(new Error(tool('pngToWebp.errors.failedToRead')));
        ...
        const errorMessage = err instanceof Error ? err.message : tool('pngToWebp.errors.conversionFailed');
        setFileQueue(prev => prev.map(f => f.id === file.id ? {
          ...f,
  ...
  const handleFileUpload = useCallback(async (files: File[]) => {
    ...
    if (!shouldUseBulkMode && files.length === 1) {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : tool('pngToWebp.errors.failedToConvert');
      ...
      const validFiles = files.filter(file => file.type.match(/^image\/png$/i) && file.size <= 10 * 1024 * 1024);

      if (validFiles.length === 0) {
        setError(tool('pngToWebp.errors.invalidFile'));
        return;
      }
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(tool('pngToWebp.errors.failedToCreateZip'));
    }
  ...
  return (
    <div className="space-y-6">
      <BaseBulkConverter
        title={tool('pngToWebp.title')}
        description={tool('pngToWebp.description')}
        uploadAreaLabel={tool('pngToWebp.labels.uploadArea')}
        dragDropLabel={tool('pngToWebp.labels.dragDrop')}
        supportedFormatsLabel={tool('pngToWebp.labels.supportedFormats')}
        clearText={common('buttons.clear')}
        downloadAllText={common('buttons.download')}
        downloadFileName={tool('pngToWebp.downloadFileName')}
      ...
      >
        {/* PNG to WebP Options Accordion */}
        <Accordion className="w-full">
          <AccordionItem 
            title={tool('pngToWebp.options.conversionSettings')}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('pngToWebp.options.quality')}</h3>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('pngToWebp.options.compression')}</h3>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['none', 'fast', 'best'] as const).map((compression) => (
                    <Button
                      ...
                      className="w-full"
                    >
                      {tool(`pngToWebp.options.compressionLevels.${compression}`)}
                    </Button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/50">
                  ...
                  <h3 className="text-base font-semibold text-foreground">{tool('pngToWebp.options.webpSettings')}</h3>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                    ...
                    >
                      {tool('pngToWebp.options.losslessMode')}
                    </label>
                    <Switch
                      id="lossless"
                  ...
                  <p className="text-xs text-muted-foreground">
                    {tool('pngToWebp.options.losslessModeDescription')}
                  </p>
                </div>
              </div>
            </div>
