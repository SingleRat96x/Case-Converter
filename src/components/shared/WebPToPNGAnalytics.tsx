...
interface WebPToPNGAnalyticsProps {
...
export function WebPToPNGAnalytics({ 
  originalFile,
  processedFile,
  variant = 'default', 
  showTitle = true,
  className = '' 
}: WebPToPNGAnalyticsProps) {
  const { tool } = useToolTranslations('tools/image-tools');
  
  // Calculate WebP to PNG specific statistics - always show cards
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    ...
    stats.push({
      key: 'originalSize',
      label: tool('webpToPng.stats.originalSize'),
    ...
    stats.push(
      {
        key: 'newSize',
        label: tool('webpToPng.stats.newSize'),
      ...
      {
        key: 'sizeChange',
        label: tool('webpToPng.stats.sizeChange'),
      ...
      },
      {
        ...
        label: tool('webpToPng.stats.sizeRatio'),
      ...
      {
        key: 'quality',
        label: tool('webpToPng.stats.quality'),
        value: 'Lossless',
        icon: Zap,
      ...
      {
        key: 'format',
        label: tool('webpToPng.stats.format'),
        value: 'PNG',
        icon: Activity,
