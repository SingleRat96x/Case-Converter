...
import { generateSHA1Hash, formatSHA1Hash } from '@/lib/sha1Utils';

interface SHA1HashAnalyticsProps {
  text: string;
  showTitle?: boolean;
...
export function SHA1HashAnalytics({ text, showTitle = true, variant = 'default' }: SHA1HashAnalyticsProps) {
  const { tool } = useToolTranslations('tools/miscellaneous');
  
  const stats: SHA1Stats = useMemo(() => {
    ...
    const hash = generateSHA1Hash(text);
    const formats = formatSHA1Hash(hash);
  ...
  const statisticsData = [
    {
      key: 'inputLength',
      label: tool('sha1Hash.analytics.inputLength', 'Input Length'),
    ...
    {
      key: 'hashLength',
      label: tool('sha1Hash.analytics.hashLength', 'Hash Length'),
    ...
    {
      key: 'encoding',
      label: tool('sha1Hash.analytics.encoding', 'Encoding'),
      value: 'UTF-8',
      icon: Key,
      ...
      {showTitle && (
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            {tool('sha1Hash.stats.title', 'SHA-1 Hash Analysis')}
          </CardTitle>
        </CardHeader>
      )}
      <CardContent>
