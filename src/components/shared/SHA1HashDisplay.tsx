...
export type SHA1Format = 'lowercase' | 'uppercase' | 'colonSeparated';

interface SHA1HashDisplayProps {
  hash: string;
  format: SHA1Format;
  isAnimating?: boolean;
}

export function SHA1HashDisplay({
  hash,
  format,
  isAnimating = false
}: SHA1HashDisplayProps) {
  const { tool } = useToolTranslations('tools/miscellaneous');
  const [showCopied, setShowCopied] = useState(false);
  
  const formatHash = (hashValue: string, formatType: SHA1Format): string => {
    if (!hashValue) return '';
      ...
      {showCopied && (
        <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 z-10">
          <div className="bg-green-500 text-white px-3 py-1 rounded-md text-sm font-medium shadow-lg flex items-center gap-2">
            ...
            {tool('sha1Hash.formats.copied', 'Hash copied to clipboard!')}
          </div>
        </div>
      )}
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center space-y-3">
              ...
              <div className="text-muted-foreground text-lg font-medium">
                {tool('sha1Hash.inputPlaceholder', 'Enter text to generate SHA-1 hash...')}
              </div>
              <div className="text-sm text-muted-foreground/70">
                {tool('sha1Hash.outputPlaceholder', 'Your SHA-1 hash will appear here')}
              </div>
            </div>
          </div>
        )}
