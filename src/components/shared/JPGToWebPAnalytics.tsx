...
interface JPGToWebPAnalyticsProps {
...
export function JPGToWebPAnalytics({ 
  originalFile,
  processedFile,
  variant = 'default', 
  showTitle = true,
  className = '' 
}: JPGToWebPAnalyticsProps) {
  const { tool } = useToolTranslations('tools/image-tools');
  
  // Calculate JPG to WebP specific statistics - always show cards
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    ...
    stats.push({
      key: 'originalSize',
      label: tool('jpgToWebp.stats.originalSize'),
    ...
    stats.push(
      {
        key: 'newSize',
        label: tool('jpgToWebp.stats.newSize'),
      ...
      {
        key: 'sizeChange',
        label: tool('jpgToWebp.stats.sizeChange'),
      ...
      },
      {
        ...
        label: tool('jpgToWebp.stats.sizeRatio'),
      ...
      {
        key: 'quality',
        label: tool('jpgToWebp.stats.quality'),
        value: 'High',
        icon: Zap,
      ...
      {
        key: 'format',
        label: tool('jpgToWebp.stats.format'),
        value: 'WebP',
        icon: Activity,
