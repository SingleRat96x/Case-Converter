...
interface PNGToWebPAnalyticsProps {
...
export function PNGToWebPAnalytics({ 
  originalFile,
  processedFile,
  variant = 'default', 
  showTitle = true,
  className = '' 
}: PNGToWebPAnalyticsProps) {
  const { tool } = useToolTranslations('tools/image-tools');
  
  // Calculate PNG to WebP specific statistics - always show cards
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    ...
    stats.push({
      key: 'originalSize',
      label: tool('pngToWebp.stats.originalSize'),
    ...
    // Always show WebP size (with placeholder if no processed file)
    ...
    stats.push(
      {
        key: 'newSize',
        label: tool('pngToWebp.stats.newSize'),
      ...
      {
        key: 'sizeChange',
        label: tool('pngToWebp.stats.sizeChange'),
      ...
      },
      {
        ...
        label: tool('pngToWebp.stats.sizeRatio'),
      ...
      {
        key: 'quality',
        label: tool('pngToWebp.stats.quality'),
        value: 'High',
        icon: Zap,
      ...
      {
        key: 'format',
        label: tool('pngToWebp.stats.format'),
        value: 'WebP',
        icon: Activity,
