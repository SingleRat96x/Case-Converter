...
interface PNGToJPGAnalyticsProps {
...
export function PNGToJPGAnalytics({ 
  originalFile,
  processedFile,
  variant = 'default', 
  showTitle = true,
  className = '' 
}: PNGToJPGAnalyticsProps) {
  const { tool } = useToolTranslations('tools/image-tools');
  
  // Calculate PNG to JPG specific statistics - always show cards
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    ...
    stats.push({
      key: 'originalSize',
      label: tool('pngToJpg.stats.originalSize'),
    ...
    stats.push(
      {
        key: 'newSize',
        label: tool('pngToJpg.stats.newSize'),
      ...
      {
        key: 'sizeChange',
        label: tool('pngToJpg.stats.sizeChange'),
      ...
      },
      {
        ...
        label: tool('pngToJpg.stats.sizeRatio'),
      ...
      {
        key: 'quality',
        label: tool('pngToJpg.stats.quality'),
        value: 'High',
        icon: Zap,
      ...
      {
        key: 'format',
        label: tool('pngToJpg.stats.format'),
        value: 'JPG',
        icon: Activity,
