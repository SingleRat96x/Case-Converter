...
interface WebPToJPGAnalyticsProps {
...
export function WebPToJPGAnalytics({ 
  originalFile,
  processedFile,
  variant = 'default', 
  showTitle = true,
  className = '' 
}: WebPToJPGAnalyticsProps) {
  const { tool } = useToolTranslations('tools/image-tools');
  
  // Calculate WebP to JPG specific statistics - always show cards
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    ...
    stats.push({
      key: 'originalSize',
      label: tool('webpToJpg.stats.originalSize'),
    ...
    // Always show JPG size (with placeholder if no processed file)
    ...
    stats.push(
      {
        key: 'newSize',
        label: tool('webpToJpg.stats.newSize'),
      ...
      {
        key: 'sizeChange',
        label: tool('webpToJpg.stats.sizeChange'),
      ...
      },
      {
        ...
        label: tool('webpToJpg.stats.sizeRatio'),
      ...
      {
        key: 'quality',
        label: tool('webpToJpg.stats.quality'),
        value: 'High',
        icon: Zap,
      ...
      {
        key: 'format',
        label: tool('webpToJpg.stats.format'),
        value: 'JPG',
        icon: Activity,
