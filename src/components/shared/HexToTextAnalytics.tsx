...
export function HexToTextAnalytics({ 
  input, 
  output, 
  variant = 'default', 
  showTitle = true,
  ...
  // Calculate hex-specific statistics
  const analyticsData = React.useMemo((): AnalyticsData[] => {
    const inputChars = input.length;
    const outputHexChars = output.replace(/[\s:0x]/g, '').length;
    const hexBytes = Math.ceil(outputHexChars / 2);
    const compressionRatio = inputChars > 0 ? (outputHexChars / (inputChars * 2)) : 0;
    const hexDensity = outputHexChars > 0 ? (outputHexChars / hexBytes) : 0;
    ...
    return [
      ...
      {
        key: 'hexChars',
        label: 'Hex Characters',
        value: outputHexChars,
        icon: Hash,
