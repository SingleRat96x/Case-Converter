...
export default function PhoneticSpellingConverter() {
  ...
  const convertToPhonetic = (text: string): string => {
    return text
      ...
      .split('')
      .map(char => {
        if (PHONETIC_SPELLING[char]) {
          return PHONETIC_SPELLING[char];
        } else if (char === ' ') {
          return 'space';
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(convertToPhonetic(inputText));
  };
    ...
    <div className="max-w-[900px] mx-auto space-y-4">
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            readOnly
            value={convertToPhonetic(inputText)}
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
        ...
        <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
          ...
          <div>• Useful for spelling out text over the phone or radio</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 border-t border-gray-200 dark:border-gray-800 pt-4">
        ...
        <span className="text-gray-400 dark:text-gray-600">|</span>
        <span>Symbols: {stats.symbols}</span>
      </div>
    </div>
  );
} 