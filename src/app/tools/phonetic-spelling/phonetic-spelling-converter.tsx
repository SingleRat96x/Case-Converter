...
export function PhoneticSpellingConverter() {
  ...
  const convertToPhoneticSpelling = (text: string) => {
    return text
      .toUpperCase()
      .split('')
      .map(char => {
        if (phoneticMap[char]) {
          return phoneticMap[char];
        } else if (char === ' ') {
          return '\n';
        } else {
          return char;
  ...
  const handleDownload = () => {
    const blob = new Blob([convertToPhoneticSpelling(inputText)], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'phonetic-spelling.txt';
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(convertToPhoneticSpelling(inputText));
  };
  ...
  return (
    <div className="max-w-[900px] mx-auto space-y-4">
      <div className="grid gap-6 md:grid-cols-2">
        ...
        {/* Output */}
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-900 dark:text-gray-50">Phonetic Spelling Result</label>
          <textarea
            className="w-full min-h-[300px] p-4 rounded-lg border bg-gray-50 dark:bg-gray-900 resize-y text-gray-900 dark:text-gray-100"
            readOnly
            value={convertToPhoneticSpelling(inputText)}
          />
        </div>
      </div>
