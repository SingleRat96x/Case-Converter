...
export function TitleCaseConverter() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(convertToTitleCase(inputText));
  };
    ...
    <div className="max-w-[900px] mx-auto space-y-8">
      <div className="space-y-4">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {/* Output */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-900 dark:text-gray-50">Title Case Result</label>
            <textarea
              className="w-full min-h-[300px] p-4 rounded-lg border bg-gray-50 dark:bg-gray-900 resize-y text-gray-900 dark:text-gray-100"
              readOnly
              value={convertToTitleCase(inputText)}
            />
          </div>
        </div>
